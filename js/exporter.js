...
(function () {
  ...
  function getSlideshowCSS() {
    return [
      '.slideshow-nav { position: fixed; top: 0; left: 0; right: 0; height: 48px; display: flex; align-items: center; justify-content: space-between; padding: 0 24px; background: linear-gradient(to bottom, rgba(0,0,0,0.7), transparent); z-index: 100; opacity: 0; pointer-events: none; transition: opacity 0.3s ease; }',
      '.slideshow-nav.ui-visible { opacity: 1; pointer-events: auto; }',
      ...
      '.slideshow-dots { position: fixed; bottom: 16px; left: 50%; transform: translateX(-50%); display: flex; gap: 8px; z-index: 100; opacity: 0; pointer-events: none; transition: opacity 0.3s ease; }',
      '.slideshow-dots.ui-visible { opacity: 1; pointer-events: auto; }',
  ...
  function getSlideshowJS(total) {
    return [
      ...
      '  var vw = window.innerWidth - 24, vh = window.innerHeight - 64;',
      '  var sw = 960, sh = 540;',
      '  // No upper cap: slide scales to fit the viewport. On 2K / 4K / 5K',
      '  // displays the slide now fills the screen instead of leaving a',
      '  // huge black border. Bottom end keeps a small margin via the',
      '  // viewport subtraction above so nav/dots stay clear of the slide.',
      ...
      'var uiTimer = null, keyNavMode = false, mouseAccum = 0;',
      'var MOUSE_THRESHOLD = 20;',
      'function showUi() {',
      '  var nav = document.querySelector(".slideshow-nav");',
      '  var dots = document.getElementById("dots");',
      '  if (nav) nav.classList.add("ui-visible");',
      '  if (dots) dots.classList.add("ui-visible");',
      ...
      '  var nav = document.querySelector(".slideshow-nav");',
      '  var dots = document.getElementById("dots");',
      '  if (nav) nav.classList.remove("ui-visible");',
      '  if (dots) dots.classList.remove("ui-visible");',
      '  clearTimeout(uiTimer);',
      '  uiTimer = null;',
      '}',
      'document.addEventListener("mousemove", function(e) {',
      '  if (keyNavMode) {',
      '    mouseAccum += Math.abs(e.movementX || 0) + Math.abs(e.movementY || 0);',
      '    if (mouseAccum >= MOUSE_THRESHOLD) { keyNavMode = false; }',
      '    return;',
      '  }',
      '  showUi();',
      '});',
      'document.addEventListener("keydown", function(e) {',
      '  if (e.key === "ArrowRight" || e.key === " ") { e.preventDefault(); keyNavMode = true; mouseAccum = 0; hideUi(); next(); }',
      '  else if (e.key === "ArrowLeft") { e.preventDefault(); keyNavMode = true; mouseAccum = 0; hideUi(); prev(); }',
